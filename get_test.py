...
def create_pdf_2(test_data, answers_array, result_test, from_user_username, from_user_id, test_name, user_name, doc_name):
    ...
    # Добавляем информацию о тестируемом, лечащем враче и клинике
    add_info(f"{test_name}", 180, y_position, "DejaVu-Bold", 15, black)
    y_position -=50
    add_info(f"Имя тестируемого:", 45, y_position, "DejaVu-Bold", 10, black)
    y_position += 20
    add_info(f"{user_name}", 180, y_position, "DejaVu", 9, black)
    ...
    # Добавляем логотип клиники
    c.drawImage('logo.png', 45, 710, width=94, height=41)
...
async def get_answer(web_app_message):
    ...
    pdf_file = create_pdf_2(file_data, answers_array, result_test, from_user_username, from_user_id, full_test_name, user_name, doc_name)
