...
def create_pdf(test_data, answers_array, result_test, from_user_username, from_user_id, test_name, user_name, doc_name):
    ...
    add_info(f"{test_name}", 350, y_position,
             "helvetica-Neue", 17, black, "center")
    y_position -= 50
    ...
    add_info(f"Лечащий врач:", 45, y_position,
             "helvetica-Neue", 10, black)  # Поле для заполнения
    y_position += 20
    ...
    for scale, score in result_test.items():
        ...
        y_position += 20
        add_info(f"{score}", 290, y_position, "helvetica-AG", 9, blue)
    ...
    for i, answer_dict in enumerate(answers_array, start=1):
        ...
        if test_name == 'Опросник гипомании HCL 32':
            ...
            if question_number + 1 == 1:
                add_info(f"{section['section']}", 70,
                         y_position, "helvetica-Neue", 11, black)
            add_info(f"Вопрос {question_number + 1}: {question_text}",
                     70, y_position, "helvetica-Neue", 10, blue)
            add_info(f"Ответ: {answer_text}", 100,
                     y_position, "helvetica-AG", 10, black)
        else:
...
def get_result_test_scl(answersArray, test_data):
    ...
    psi_count = sum(1 for item in answersArray if 1 <=
                    int(item["answer"]) <= 4)

    # Расчет индекса выраженности дистресса PDSI
    pdsi_index = round((gsi_index * len(answersArray)) /
                       psi_count if psi_count != 0 else 0, 2)
...
def get_result_test_was(answersArray, test_data):
    ...
    for scale, items in scales.items():
        ...
        # Сохраняем округленное значение в словаре для данной шкалы
        scale_scores[scale] = average_score
...
async def get_answer(web_app_message):
    ...
    if (test_name == 'SCL_90_R'):
        result_test, result_string = get_result_test_scl(
    ...
    pdf_file = create_pdf(file_data, answers_array, result_test,
